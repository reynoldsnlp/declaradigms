...
function stripFlagDiacritics(form) {
...
function stringifyLookupOutput(raw) {
  ...
  return raw
    .map((item) => {
      ...
      if (Array.isArray(first)) return stripFlagDiacritics(first.join(''));
      if (typeof first === 'string') return stripFlagDiacritics(first);
      if (first == null) return null;
      return stripFlagDiacritics(String(first));
    })
    .filter(Boolean);
}
    ...
    for (const table of section.tables) {
      ...
      for (const row of table.rows) {
        ...
        row.cells.forEach((rawCell, idx) => {
          ...
          const colspan = outputRow.colspan[idx] || 1;
      ...
      table.rows.forEach((row) => {
        ...
        row.cells.forEach((cell) => {
          ...
          if (!cell.variants.length) {
            ...
            const renderedVariants = cell.variants
              .map((variant) => {
                const readingAttrs = dataAttrs({
                  'data-reading': variant.inputUsed || model.reading
                });

                if (Array.isArray(variant.forms) && variant.forms.length) {
                  return variant.forms
                    .map(
                      (form) => `<span${classAttr(variant.styleClass)}${readingAttrs}>${escapeHtml(form)}</span>`
                    )
                    .join(', ');
                }

                return `<span${classAttr(variant.styleClass)}${readingAttrs}>${escapeHtml(variant.value || placeholder)}</span>`;
              })
              .join('<br>');

            html += renderedVariants;
          }
